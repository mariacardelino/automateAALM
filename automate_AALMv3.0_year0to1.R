...
## RUN THIS FOR AGE 0-1

#BEFORE MODEL: read text file

text_path <- "<file_path_to_input_file>" #Define the filepath for input parameters, 0-1 year

par_df <- read_table(text_path, col_names = FALSE)
print(head(par_df))
#120 rows, 12 variables (columns) for year 0-1

file_path <- "<file_path_to_model>" # Define the file path for model (ends in /AALMv3-0_030124.xlsm)

# Create a COM object for Excel
excel <- RDCOMClient::COMCreate("Excel.Application")
...
#### START LOOP ################################## 
# COLUMN ORDER FOR TEXT FILE 0-1 year:
#1 BLDMOT
#2 RDIFF
#3 RCS2DF
...
#6 RRBC0 - 0 to 3 months
#7 RRBC03 - 3 to 12 months

#8 SOIL0 - 0-12 months
#9 DUST0 - 0-12 months
#10 WATER0 - 0-12 months
#11 AIR0 - 0-12 months
#12 FOOD0 - 0-12 months
...
age_range[["Value"]] <- paste("1") #change
#gender_range[["Value"]] <- paste("Male")

for (x in 1:120) { #for each row of the df, run the model
...
library(tidyverse)
#desired output data
output_year0to1 <- combined_df %>%
  filter(Time_years == 1.000) %>%
  select(Iteration, Time_years, Blood_Pb, Bone_Pb, X1, X2, X3, X4, X5, X6, X7, X8, X9, X10, X11, X11.1)


############ Write dataframe to a TXT file, CHANGE NAME HERE ################
write.table(output_year0to1, "<output_file_name>", row.names = FALSE, sep = "\t") #write output file name
