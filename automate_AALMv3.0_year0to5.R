...
print(head(par_df))
#290 rows, 29 variables (columns) for year 0-5
...
#COLUMN ORDER FOR TEXT FILE 0-5 YEAR:
# 
# 1. RDIFF
# 2. RCS2DF
# 3. RCORT1
...
age_range[["Value"]] <- paste("5") #change for each age
gender_range[["Value"]] <- paste("Male")

for (x in 1:290) { #for each row of the df, run the model
...
library(tidyverse)
#desired output data
output_year0to5 <- combined_df %>%
  filter(Time_years == 5.000) %>% #change for each age
  select(Iteration, Time_years, Blood_Pb, Bone_Pb, X1, X2, X3, X4, X5, X6, X7, X8, X9, X10, X11, X12, X13, X14, X15, X16, X17, X18, X19, X20, X21, X22, X23, X24, X25, X26, X27, X28, X29)

############ Write dataframe to a TXT file, CHANGE NAME HERE ################
write.table(output_year0to5, "<output_file_name>", row.names = FALSE, sep = "\t") #write output file name
